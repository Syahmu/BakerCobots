classdef TrayBot < handle
    ...
    methods%% Class for UR5 robot simulation
        function self = TrayBot(useGripper)
            if nargin < 1
                useGripper = false;
            end
        ...
        function PlotAndColourRobot(self)%robot,workspace)
            for linkIndex = 0:self.model.n
                if self.useGripper && linkIndex == self.model.n
                    [ faceData, vertexData, plyData{linkIndex+1} ] = plyread(['Tray_',num2str(linkIndex),'Gripper.ply'],'tri'); %#ok<AGROW>
                else
                    [ faceData, vertexData, plyData{linkIndex+1} ] = plyread(['Tray_',num2str(linkIndex),'.ply'],'tri'); %#ok<AGROW>
                end
